...
const localFaultTreeInference = computed(() => buildFaultTreeInference())
...
const balanceVerification = computed(() => {
  const equation = activeResult.value?.balance?.formulaText || 'EGOX = GOX - 125/136 × LIN + 114/136 × (GAR - GARBase)；PAIR、GAN、FIC1、FAR、CAR、AIR由外压缩.sfit模型输出。'
...
const materialBalanceIOItems = computed(() => {
  ...
  const keyVariables = modelVariables
    .filter((item) => ['EGOX', 'PAIR', 'GAN', 'FIC1_model', 'FAR', 'CAR', 'AIR'].includes(item.key))
    .slice(0, 8)
  ...
  return [
    ...
    { label: '输入', value: 'GOX(FIQC102)、LIN(FIC8)、GAR(FI131)、GARBase、FI702、FIC701、FIC1。' },
...
const materialBalanceTrace = computed(() => {
  ...
  return {
    ...
    steps: [
      ...
      { key: 'pair', index: '03', title: '回归 PAIR', value: format('PAIR'), note: '由 .sfit poly2 计算进塔分离空气' },
...
const directionalRiskRows = computed(() => {
  ...
  const focusOrder = ['AI705', 'AI701', 'FI702', 'FIQC701', 'FIC101', 'FIQC102', 'FIC103', 'FI131', 'FIC8', 'FI105', 'FIC1', 'PdI2', 'PdI1', 'AIAS102', 'V3', 'BALANCE']
...
const agentEvidenceNodes = computed(() => {
  const rows = buildLocalEvidenceNodes()
...
const agentFaultTreePath = computed(() => {
  return buildLocalFaultTreePath().filter(Boolean).join('\n')
})
const agentBranchRanking = computed(() => {
  const rows = buildLocalBranchRanking()
...
function buildAnalysisStepSummary(stageId) {
  ...
  if (stageId === 'material_balance') {
    ...
    return `已按外压缩.sfit模型复核：${modelOutput || 'EGOX、PAIR、GAN、FIC1、FAR、CAR、AIR链式输出'}；方向证据：${abnormalText}${estimatedNote}。`
  }
...
function buildAnalysisStepProcessItems(stageId, state) {
  ...
  if (stageId === 'material_balance') {
    ...
    return [
      { label: '输入', text: '窗口均值/中位数后的GOX、LIN、GAR(FI131)、GARBase、FI702、FIC701、FIC1。' },
      { label: '计算', text: balance?.formulaText || balanceVerification.value.equation },
      { label: '建模', text: (externalModel.steps || []).slice(0, 4).join(' → ') || '使用交付包导出的.sfit系数。' },
      { label: '输出', text: `${externalModel.primaryOutputText || balance?.summary || balanceVerification.value.result}；方向证据：${abnormalText}。` },
      { label: '缺项', text: missingCore.length ? missingCore.map((item) => item.label).join('、') : `Demo核心输入已满足${recovered.length ? `（${recovered.join('、')}）` : ''}；正式上线建议确认FI131口径和固定GARBase。` }
    ]
  }
...
function buildAgentAnalysisRequest(detailLevel = 'full', analysisStage = '') {
  ...
  return {
    ...
    cause_branches: buildLocalBranchRanking().map((branch) => ({
      id: branch.branch?.slice(0, 1),
      branch: branch.branch,
      name: branch.status,
      confidence: branch.status,
      advice: branch.next_step,
      reason: branch.reason
    })),
    missing_information: buildLocalKeyMissingData().map((item) => item.data || item),
...
function getDirectionalCheck(tag) {
  return (activeResult.value?.directionalChecks || []).find((item) => item.tag === tag) || null
}

function getBalanceResidual(key) {
  const externalModel = activeResult.value?.balance?.outputs?.external_model || activeResult.value?.balance?.externalModel || {}
  return (externalModel.residuals || []).find((item) => item.key === key) || null
}

function checkDirection(tag, direction) {
  return getDirectionalCheck(tag)?.direction === direction
}

function checkTriggered(tag) {
  return Boolean(getDirectionalCheck(tag)?.triggered || hasDiagnosisTag(tag))
}

function checkText(tag) {
  const check = getDirectionalCheck(tag)
  if (!check) return `${displayTagCode(tag)} 缺少当前窗口或基准数据`
  return `${displayTagCode(tag)} ${check.directionText || '待判别'}，当前 ${formatMetricValue(check.currentMean)}，基准 ${formatMetricValue(check.baselineMean)}，偏差 ${formatPercent((check.deviationPct || 0) / 100)}`
}

function residualText(key) {
  const item = getBalanceResidual(key)
  if (!item) return `${key} 缺少模型/实测对比`
  return `${key} 实测 ${formatMetricValue(item.measured)}，模型 ${formatMetricValue(item.model)}，偏差 ${formatPercent((item.deviationPct || 0) / 100)}，${item.semantic || '待判别'}`
}

function scoreStatus(score, maxScore, hasHardReject = false, hasMissing = false) {
  if (hasHardReject && score <= 0) return '不支持'
  if (score >= Math.max(3, maxScore * 0.6)) return '强支持'
  if (score > 0) return '部分支持'
  if (hasMissing) return '待补证'
  return '不支持'
}

function buildFaultNode({ id, branch, name, score = 0, maxScore = 4, hardReject = false, missing = false, evidence = [], reject = [], missingData = [], nextStep = '' }) {
  const status = scoreStatus(score, maxScore, hardReject, missing || missingData.length > 0)
  const supportText = evidence.length ? `支持证据：${evidence.join('；')}` : ''
  const rejectText = reject.length ? `排除/削弱：${reject.join('；')}` : ''
  const missingText = missingData.length ? `待补证：${missingData.join('、')}` : ''
  const reason = [supportText, rejectText, missingText].filter(Boolean).join('。') || '当前数据未形成明确证据。'
  return {
    id,
    branch,
    name,
    ...
    missingData,
    reason,
    next_step: nextStep || (missingData.length ? `补充${missingData[0]}后复核。` : '继续联看趋势和物料平衡偏差。')
  }
}

function buildFaultTreeInference() {
  const result = activeResult.value
  const shape = result?.ai705Primary || {}
  const hasT0 = Boolean(result?.suspected || shape.triggered || hasDiagnosisTag('AI705'))
  const carResidual = getBalanceResidual('CAR')
  const farResidual = getBalanceResidual('FAR')
  const airResidual = getBalanceResidual('AIR')
  const ganResidual = getBalanceResidual('GAN')
  const carLow = Number.isFinite(carResidual?.deviationPct) && carResidual.deviationPct < -1.25
  const farAbnormal = Number.isFinite(farResidual?.deviationPct) && Math.abs(farResidual.deviationPct) > 1.25
  const airHigh = Number.isFinite(airResidual?.deviationPct) && airResidual.deviationPct > 1.25
  const ganNormal = Number.isFinite(ganResidual?.deviationPct) && Math.abs(ganResidual.deviationPct) <= 1.25
  const stableAir = !result?.operatingMode?.dynamic && !checkTriggered('FIC101')

  const a1 = buildFaultNode({
    id: 'A1', branch: 'A 主塔问题', name: '氧气取出量过大',
    score: (checkDirection('FIQC102', 'high') ? 2 : 0) + (checkDirection('AIAS102', 'low') ? 1 : 0) + (checkDirection('AI701', 'high') ? 1 : 0),
    hardReject: !checkDirection('FIQC102', 'high'),
    evidence: [checkDirection('FIQC102', 'high') ? checkText('FIQC102') : '', checkDirection('AIAS102', 'low') ? checkText('AIAS102') : '', checkDirection('AI701', 'high') ? checkText('AI701') : ''].filter(Boolean),
    reject: [!checkDirection('FIQC102', 'high') ? `氧气取出量未偏高：${checkText('FIQC102')}` : '', checkDirection('AIAS102', 'low') ? '' : `氧纯度未给出下降证据：${checkText('AIAS102')}`].filter(Boolean),
    nextStep: '若怀疑A1，复核氧气取出量是否超过设计/额定105%，并联看AIAS102和氩馏分含氮。'
  })
  const a2 = buildFaultNode({
    id: 'A2', branch: 'A 主塔问题', name: 'V3阀开度过大',
    score: (checkDirection('V3', 'high') ? 3 : 0) + (checkDirection('AIAS102', 'low') ? 1 : 0),
    hardReject: !checkDirection('V3', 'high'),
    evidence: [checkDirection('V3', 'high') ? checkText('V3') : '', checkDirection('AIAS102', 'low') ? checkText('AIAS102') : ''].filter(Boolean),
    reject: [!checkDirection('V3', 'high') ? `V3未偏高：${checkText('V3')}` : ''].filter(Boolean),
    nextStep: '核对V3是否在氮塞前30min出现2%-3%以上增开。'
  })
  const a3 = buildFaultNode({
    id: 'A3', branch: 'A 主塔问题', name: '膨胀空气量过大/旁通不足',
    score: (checkDirection('FIC1', 'high') ? 2 : 0) + (checkDirection('FI105', 'high') ? 2 : 0) + (checkDirection('AIAS102', 'low') ? 1 : 0),
    hardReject: !checkDirection('FIC1', 'high') && !checkDirection('FI105', 'high'),
    evidence: [checkDirection('FIC1', 'high') ? checkText('FIC1') : '', checkDirection('FI105', 'high') ? checkText('FI105') : ''].filter(Boolean),
    reject: [!checkDirection('FIC1', 'high') ? `进上塔膨胀空气未偏高：${checkText('FIC1')}` : '', !checkDirection('FI105', 'high') ? `膨胀机增压端空气未偏高：${checkText('FI105')}` : ''].filter(Boolean),
    nextStep: '若怀疑A3，补旁通阀开度/V6/V450，并看膨胀空气是否超过加工空气12%-15%。'
  })
  const a4 = buildFaultNode({
    id: 'A4', branch: 'A 主塔问题', name: '氮气/污氮气取出量过小',
    score: (checkDirection('FIC103', 'low') ? 2 : 0) + (checkDirection('FI131', 'low') ? 1 : 0) + (checkDirection('PdI2', 'high') || checkDirection('PdI1', 'high') ? 1 : 0),
    hardReject: !checkDirection('FIC103', 'low') && !checkDirection('FI131', 'low'),
    evidence: [checkDirection('FIC103', 'low') ? checkText('FIC103') : '', checkDirection('FI131', 'low') ? checkText('FI131') : '', checkDirection('PdI2', 'high') ? checkText('PdI2') : ''].filter(Boolean),
    reject: [!checkDirection('FIC103', 'low') ? `产品氮气未偏低：${checkText('FIC103')}` : '', !checkDirection('FI131', 'low') ? `中压氮气外送未偏低：${checkText('FI131')}` : '', ganNormal ? residualText('GAN') : ''].filter(Boolean),
    nextStep: '若怀疑A4，补污氮取出量和上塔顶部压力，确认氮组分排出通道是否收窄。'
  })

  const b1 = buildFaultNode({
    id: 'B1', branch: 'B 粗氩塔自身', name: '粗氩冷凝器换热条件恶化',
    score: (checkDirection('FIQC701', 'low') ? 1 : 0) + (carLow ? 1 : 0) + (checkDirection('LIC701', 'high') ? 1 : 0),
    missing: true,
    evidence: [checkDirection('FIQC701', 'low') ? checkText('FIQC701') : '', carLow ? residualText('CAR') : '', checkDirection('LIC701', 'high') ? checkText('LIC701') : ''].filter(Boolean),
    reject: [],
    missingData: ['LIC702', '粗氩冷凝器温差', '粗氩塔阻力PDI701', '液空氧含量/蒸发侧压力'],
    nextStep: '补LIC702、冷凝器温差和PDI701，确认是否换热温差缩小导致冷凝能力下降。'
  })
  const b2 = buildFaultNode({
    id: 'B2', branch: 'B 粗氩塔自身', name: '粗氩抽取量/氩系统负荷不匹配',
    score: (checkDirection('FIQC701', 'low') ? 3 : 0) + (carLow ? 3 : 0) + (farAbnormal ? 1 : 0) + (hasT0 ? 1 : 0),
    evidence: [checkDirection('FIQC701', 'low') ? checkText('FIQC701') : '', carLow ? residualText('CAR') : '', farAbnormal ? residualText('FAR') : '', hasT0 ? 'AI705下凹主事件已成立' : ''].filter(Boolean),
    reject: [],
    missingData: [],
    nextStep: '优先核对粗氩抽出量与氩馏分量匹配关系，避免粗氩抽取量骤变，联看粗氩塔底部液位。'
  })
  const b3 = buildFaultNode({
    id: 'B3', branch: 'B 粗氩塔自身', name: '氩泵密封气内漏/外源氮进入氩系统',
    score: 0,
    missing: true,
    evidence: [],
    reject: [Number.isFinite(shape.minValue) && shape.minValue < 98 ? 'AI705表现为明显短时下凹，不是长期徘徊约99.1%的典型密封气内漏形态。' : ''],
    missingData: ['氩泵密封气类型/流量', '氩泵出口压力', '密封气切换试验记录'],
    nextStep: '若B2/B1不能解释，做氮气密封到氩气/自密封的对比试验。'
  })
  const c1 = buildFaultNode({
    id: 'C1', branch: 'C 前端/空气系统', name: '分子筛切换/均压导致进塔空气量振荡',
    score: (result?.operatingMode?.dynamic ? 2 : 0) + (checkTriggered('FIC101') ? 1 : 0) + (airHigh ? 1 : 0),
    hardReject: stableAir,
    evidence: [result?.operatingMode?.dynamic ? result.operatingMode.basis : '', checkTriggered('FIC101') ? checkText('FIC101') : '', airHigh ? residualText('AIR') : ''].filter(Boolean),
    reject: [stableAir ? `进塔空气量整体稳定：${checkText('FIC101')}` : ''].filter(Boolean),
    missingData: ['分子筛切换/均压记录'],
    nextStep: '补分子筛切换时刻、均压/放空阀动作和切换后15-30min主塔压力波动。'
  })
  const d1 = buildFaultNode({
    id: 'D1-D3', branch: 'D 处置不当', name: '放空/大幅调节/过早投精氩导致加重或复发',
    score: 0,
    missing: true,
    evidence: [],
    reject: [],
    missingData: ['V712开度/流量记录', 'V705投用记录', 'V3/V2操作记录', '操作票'],
    nextStep: '补操作记录判断是否存在排放不彻底、氩馏分骤减或过早投用精氩塔。'
  })

  const nodes = [a1, a2, a3, a4, b1, b2, b3, c1, d1]
  const branchDefs = [
    { id: 'A', branch: 'A 主塔问题导致氩馏分含氮量超标', nodes: [a1, a2, a3, a4], next: '若A分支回升，按氧气取出、V3、膨胀空气、氮气取出顺序复核。' },
    { id: 'B', branch: 'B 粗氩塔自身工况失调', nodes: [b1, b2, b3], next: '优先核对粗氩抽取匹配和冷凝器换热证据。' },
    { id: 'C', branch: 'C 前端/空气系统扰动触发', nodes: [c1], next: '补分子筛和进塔空气扰动记录。' },
    { id: 'D', branch: 'D 处置不当导致加重或复发', nodes: [d1], next: '补V712/V705和关键阀位操作记录。' }
  ]
  const branches = branchDefs.map((branch) => {
    const score = branch.nodes.reduce((total, node) => total + Math.max(0, node.score), 0)
    const strong = branch.nodes.filter((node) => node.status === '强支持')
    const partial = branch.nodes.filter((node) => node.status === '部分支持')
    const pending = branch.nodes.filter((node) => node.status === '待补证')
    const rejected = branch.nodes.filter((node) => node.status === '不支持')
    const status = strong.length ? '优先排查' : partial.length ? '同步排查' : pending.length ? '待补证' : '暂不支持'
    const reason = strong.length || partial.length
      ? [...strong, ...partial].map((node) => `${node.id}${node.status}：${node.reason}`).join('；')
      : rejected.length ? `${rejected.map((node) => `${node.id}不支持`).join('、')}；${pending.length ? `${pending.map((node) => node.id).join('、')}待补证` : '当前数据未形成该分支证据'}。` : '当前数据不足。'
    return {
      ...branch,
      score,
      status,
      ...
      missingData: Array.from(new Set(pending.flatMap((node) => node.missingData)))
    }
  }).sort((a, b) => b.score - a.score)

  return {
    topEvent: {
      status: hasT0 ? levelText(result?.level) : '未触发',
      logic: `AI705下凹形态${hasT0 ? '已满足' : '未满足'}；数据分支按A/B/C/D逐节点推导。`,
      summary: hasT0 ? `T0成立，优先分支为${branches[0]?.branch || '待排查'}。` : 'T0未触发，暂不进入根因定位。'
    },
    nodes,
    branches,
    missingData: Array.from(new Set(branches.slice(0, 2).flatMap((branch) => branch.missingData))).map((data) => ({ data, node: '待补证节点', purpose: '用于区分当前优先分支中的部分支持/待补证节点。' }))
  }
}

function buildLocalEvidenceNodes() {
  const inference = localFaultTreeInference.value
  const t0 = {
    evidence: 'AI705 下凹形态',
    node: 'T0 疑似氮塞',
    status: activeResult.value?.suspected ? '强触发' : '未触发',
    explanation: activeResult.value?.ai705Primary?.basis || '先确认AI705是否形成下凹、谷底和回升。'
  }
  return [t0, ...(inference.nodes || []).map((node) => ({
    evidence: node.name,
    node: node.node,
    status: node.status,
...
function buildLocalFaultTreePath() {
  ...
  const lines = [`T0 疑似氮塞：${topEventStatus}`]
  ;(inference.branches || []).forEach((branch, branchIndex) => {
    const prefix = branchIndex === (inference.branches.length - 1) ? '└─' : '├─'
    const childPrefix = branchIndex === (inference.branches.length - 1) ? '   ' : '│  '
    lines.push(`${prefix} ${branch.branch}：${branch.status}`)
    branch.nodes.forEach((node, nodeIndex) => {
      const nodePrefix = nodeIndex === branch.nodes.length - 1 ? '└─' : '├─'
      lines.push(`${childPrefix}${nodePrefix} ${node.id} ${node.name}：${node.status}`)
    })
  })
  return lines
}
...
function buildLocalBranchRanking() {
  const rows = localFaultTreeInference.value?.branches || []
  return rows.map((item, index) => ({
    ...
    evidence: item.nodes.filter((node) => ['强支持', '部分支持'].includes(node.status)).map((node) => node.id)
  }))
}

function buildLocalKeyMissingData() {
  const inferenceMissing = localFaultTreeInference.value?.missingData || []
  const base = inferenceMissing.length ? inferenceMissing : [
    { data: '粗氩塔阻力PDI701', node: 'B1', purpose: '判断粗氩塔精馏能力是否下降。' },
    { data: '粗氩冷凝器液空液位LIC702', node: 'B1', purpose: '判断冷凝器换热是否恶化。' },
    { data: '分子筛切换记录', node: 'C1', purpose: '判断是否刚发生切换、切换是否平稳。' }
  ]
...
function buildLocalAgentAnalysis() {
  ...
  return {
    ...
    material_balance_formula_name: activeResult.value?.balance?.formulaName || 'external_compression_sfit_material_balance',
    material_balance_basis: activeResult.value?.balance?.formulaText || '外压缩物料平衡.sfit：EGOX、PAIR、GAN、FIC1、FAR、CAR、AIR链式回归模型',
