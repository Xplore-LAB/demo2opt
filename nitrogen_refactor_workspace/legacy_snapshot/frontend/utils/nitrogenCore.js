...
const materialBalanceInputTags = ['FIC101', 'FIQC102', 'FIC103', 'FIC8', 'FI702', 'FIQC701', 'FI105', 'FIC1', 'V3', 'BALANCE']
const externalCompressionFormulaText = [
  'EGOX = GOX - 125/136 × LIN + 114/136 × (GAR - GARBase)',
  'PAIR = AIRCoef1 × EGOX + AIRCoef2 + AIRCoef3 × (TEMP - TEMPBase) + AIRCoef4 × (TURBINE - TURBBase)',
  'GAN + LIN = GANCoef1 × EGOX + GANCoef2 + GANCoef3 × (TEMP - TEMPBase) + GANCoef4 × (TURBINE - TURBBase)',
  'TURBBY = TURBCoef1 × EGOX + TURBCoef2 + TURBCoef3 × (TURBINE - TURBBase)',
  'FAR = FARCoef1 × PAIR + FARCoef2；CAR = CARCoef1 × PAIR + CARCoef2；LEV = LEVCoef1 × PAIR + LEVCoef2；AIR = PAIR + TURBBY'
].join('；')
const externalCompressionModelSteps = [
  '对原始数据做60分钟均值处理。',
  '按MS1201/MS1202分子筛工作状态修正空气量漂移。',
  '按液氮阀门V8估算LIN。',
  '计算等效全液氧工况氧气产量EGOX。',
  '建立EGOX与PAIR、GAN+LIN、TURBINE/FIC1/V3的回归模型。',
  '建立PAIR与FI702、FIC701、粗氩冷凝器液位的回归模型。',
  '输出PAIR、TURBBY、AIR和粗氩侧目标量，再与当前测点做偏差语义化。'
]
...
function linearModel(coeffs, x, base = 0, shift1 = 0, shift2 = 0) {
  if (!Number.isFinite(x)) return NaN
  return (Number(coeffs?.coef1) || 0) * x
    + (Number(coeffs?.coef2) || 0)
    + (Number(coeffs?.coef3) || 0) * shift1
    + (Number(coeffs?.coef4) || 0) * shift2
    + base
}
...
function buildExternalCompressionBalanceModel(directionalChecks) {
  ...
  const GARRaw = readCurrent('FIC103')
  ...
  const GAR = firstFinite(GARRaw, readBase('FIC103'), Number.isFinite(totalAirMeasured) ? Math.max(0, totalAirMeasured * dryAirComposition.N2 - (Number.isFinite(LIN) ? LIN : 0)) : NaN)
  const GARBaseRaw = readBase('FIC103')
  const GARBase = firstFinite(GARBaseRaw, readPrevious('FIC103'), Number(baseParameters.GARBase), GAR)
  const TURBINE = readCurrent('FIC1')
  const TURBBase = firstFinite(readBase('FIC1'), readPrevious('FIC1'), Number(baseParameters.TURBBase), TURBINE)
  const TEMP = firstFinite(readCurrent('TEMP'), readBase('TEMP'), Number(baseParameters.TEMPBase))
  const TEMPBase = firstFinite(Number(baseParameters.TEMPBase), TEMP)
  const TURBBYMeasured = readCurrent('FI105')
  const missingCoreInputs = [
    ['GOX', GOX, 'FIQC102氧气产量'],
    ['LIN', LIN, 'FIC8液氮流量或V8估算'],
    ['GAR', GAR, 'FIC103产品氮气流量'],
    ['GARBase', GARBase, '同负荷历史/前段氮气基准']
  ...
  const tempShift = Number.isFinite(TEMP) && Number.isFinite(TEMPBase) ? TEMP - TEMPBase : 0
  const turbineShift = Number.isFinite(TURBINE) && Number.isFinite(TURBBase) ? TURBINE - TURBBase : 0
  const PAIRModel = hasCoreInputs ? linearModel(coefficients.AIR, EGOX, 0, tempShift, turbineShift) : NaN
  const GANLINModel = hasCoreInputs ? linearModel(coefficients.GAN, EGOX, 0, tempShift, turbineShift) : NaN
  const TURBBYModel = hasCoreInputs ? linearModel(coefficients.TURB, EGOX, 0, turbineShift, 0) : NaN
  const FARModel = Number.isFinite(PAIRModel) ? linearModel(coefficients.FAR, PAIRModel) : NaN
  const CARModel = Number.isFinite(PAIRModel) ? linearModel(coefficients.CAR, PAIRModel) : NaN
  const LEVModel = Number.isFinite(PAIRModel) ? linearModel(coefficients.LEV, PAIRModel) : NaN
  const AIRModel = Number.isFinite(PAIRModel) && Number.isFinite(TURBBYModel) ? PAIRModel + TURBBYModel : NaN
  const PAIRActual = Number.isFinite(totalAirMeasured) && Number.isFinite(TURBBYMeasured)
    ? totalAirMeasured - TURBBYMeasured
    : NaN
  const TURBBYActual = TURBBYMeasured
  const GANLINActual = Number.isFinite(GAR) && Number.isFinite(LIN) ? GAR + LIN : NaN
  const FARActual = readCurrent('FI702')
  const CARActual = readCurrent('FIQC701')
  const LEVActual = readCurrent('LIC701')
  ...
  if (!Number.isFinite(GARRaw) && Number.isFinite(GAR)) recoveredInputs.push(Number.isFinite(readBase('FIC103')) ? 'GAR使用同负荷基准临时兜底' : 'GAR用进塔空气氮组分临时估算')
  if (!Number.isFinite(GARBaseRaw) && Number.isFinite(GARBase) && GARBase !== Number(baseParameters.GARBase)) recoveredInputs.push('GARBase使用前段/当前均值临时兜底')
  ...
  const variables = [
    ...
    { key: 'LIN', name: '液氮流量', value: LIN, source: 'FIC8 / 后续接V8估算' },
    { key: 'GAR', name: '产品氮气流量', value: GAR, source: 'FIC103' },
    { key: 'GARBase', name: '建模段氮气基准', value: GARBase, source: '同负荷历史基准' },
    { key: 'EGOX', name: '等效全液氧工况氧气产量', value: EGOX, source: '外压缩方程①' },
    { key: 'TEMP', name: '温度修正输入', value: TEMP, source: '系数文件TEMPBase / 后续TEMP测点' },
    { key: 'TEMPBase', name: '建模段温度基准', value: TEMPBase, source: '系数文件' },
    { key: 'TURBBase', name: '膨胀空气基准', value: TURBBase, source: '系数文件 / 历史基准' },
    { key: 'PAIR', name: '进塔分离空气流量', value: PAIRModel, source: '系数文件 AIRCoef1-4 / 回归模型②' },
    { key: 'PAIR_actual', name: '进塔分离空气实测估算', value: PAIRActual, source: 'FIC101 - FI105' },
    { key: 'GAN+LIN', name: '总氮', value: GANLINModel, source: '系数文件 GANCoef1-4 / 回归模型③' },
    { key: 'GAN+LIN_actual', name: '总氮实测估算', value: GANLINActual, source: 'GAR + LIN' },
    { key: 'TURBBY', name: '膨胀空气旁通量', value: TURBBYModel, source: '系数文件 TURBCoef1-3 / 回归模型④' },
    { key: 'TURBBY_actual', name: '膨胀空气旁通实测', value: TURBBYActual, source: 'FI105' },
    { key: 'FAR', name: '氩馏分流量', value: FARModel, source: '系数文件 FARCoef1-2 / 回归模型⑤' },
    { key: 'FAR_actual', name: '氩馏分流量实测', value: FARActual, source: 'FI702' },
    { key: 'CAR', name: '粗氩流量', value: CARModel, source: '系数文件 CARCoef1-2 / 回归模型⑥' },
    { key: 'CAR_actual', name: '粗氩流量实测', value: CARActual, source: 'FIQC701' },
    { key: 'LEV', name: '粗氩冷凝器液位', value: LEVModel, source: '系数文件 LEVCoef1-2 / 回归模型⑦' },
    { key: 'LEV_actual', name: '粗氩冷凝器液位实测', value: LEVActual, source: 'LIC701' },
    { key: 'AIR', name: '总空气流量', value: AIRModel, source: 'PAIR + TURBBY' },
  ...
  return {
    ...
    coefficientNote: 'AIRCoef/GANCoef/TURBCoef/FARCoef/CARCoef/LEVCoef 已从系数文件读取；LEVCoef1=0、LEVCoef2=0，所以LEV模型值会计算为0。',
    missingCoreInputs: missingCoreInputs.map(([key, , label]) => ({ key, label })),
    missingForProduction: [
      'MS1201/MS1202分子筛工作状态，用于修正空气量漂移。',
      'V8液氮阀门开度到LIN的估算系数。',
      'TEMP实时温度字段；TEMPBase已由系数文件给定为30，当前AIR/GAN温度系数为0。',
      '粗氩塔阻力、粗氩冷凝器温差等用于故障树A3/A4分支的现场测点。'
    ],
    recoveredInputs,
    formulaText: externalCompressionFormulaText,
    steps: externalCompressionModelSteps,
    correctionNotes: [
      '60分钟均值已在演示层作为建模输入口径呈现。',
      'MS1201/MS1202分子筛状态修正、V8估算LIN、TEMPBase/TURBBase后续接真实字段。',
      '回归结果要求“不偏不倚、居中而过”，当前先输出链路和方向证据。'
    ],
    inputs: { GOX, LIN, GAR, GARBase, TURBINE, TURBBase, TEMP, TEMPBase },
    variables,
    turbineShift,
    primaryOutputText: Number.isFinite(EGOX)
      ? `EGOX ${formatMaybe(EGOX)}，PAIR模型值 ${formatMaybe(PAIRModel)}，AIR模型值 ${formatMaybe(AIRModel)}，FAR模型值 ${formatMaybe(FARModel)}，CAR模型值 ${formatMaybe(CARModel)}，LEV模型值 ${formatMaybe(LEVModel)}。`
...
function evaluateMaterialBalance(stats, operatingMode, baselineContext, directionalChecks) {
  ...
  if (!stats?.valid) {
    return {
      ...
      formulaName: 'external_compression_material_balance',
      formulaText: externalCompressionFormulaText,
