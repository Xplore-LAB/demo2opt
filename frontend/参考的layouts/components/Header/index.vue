...
// import CreateChat from './components/CreateChat.vue'; // 隐藏 CreateChat 模块
...
onMounted(() => {
  ...
  if (designStore.isCollapse) {
    document.documentElement.style.setProperty(`--sidebar-left-container-default-width`, `0px`)
  }
  else {
  ...
  <div class="header-container">
    <div class="header-box relative z-10 top-0 left-0 right-0">
      <div class="absolute left-0 right-0 top-0 bottom-0 flex items-center flex-row">
        ...
        <!-- 右边 -->
        <div class="right-box flex h-full items-center pr-20px flex-shrink-0 ml-auto flex-row">
          <!-- 管理入口按钮已移除，保留侧边或弹层中的“用户管理”入口 -->
          <!-- 只有在用户完全登录且状态有效时才显示Avatar -->
          <Avatar v-show="userStore.isLogin" />
          <!-- 未登录或登录状态无效时显示登录按钮 -->
          <LoginBtn v-show="!userStore.isLogin" />
        </div>
      </div>
    </div>
  </div>
